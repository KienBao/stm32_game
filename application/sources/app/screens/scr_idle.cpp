...
void scr_idle_handle(ak_msg_t* msg) {
	...
	case AC_DISPLAY_BUTON_MODE_RELEASED: {
		...
		// SCREEN_TRAN(scr_noen_handle, &scr_noen);
		// SCREEN_TRAN(scr_happybirthday_handle, &scr_happybirthday);
		SCREEN_TRAN(scr_gameconca_menu_handle, &scr_gameconcamenu); 
	}
		break;
	...
	case AC_DISPLAY_BUTON_DOWN_RELEASED: {
		...
		if (v_idle_ball.empty()) {
			timer_remove_attr(AC_TASK_DISPLAY_ID, AC_DISPLAY_SHOW_IDLE_BALL_MOVING_UPDATE);
			SCREEN_TRAN(scr_gameUI_handle, &scr_gameUI);
		}
	}
		break;
